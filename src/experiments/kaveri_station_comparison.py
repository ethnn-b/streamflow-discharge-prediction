"""
Regenerates the qualitative-validation hydrograph/scatter figures (Figure 6/7
style) for specific Kaveri stations, reusing a single distance-only Ordered
PIMF chaining run (no connectivity matrix available for this basin). Used to
pick a representative Kaveri station for the supplementary qualitative
validation material.
"""
import os
import warnings
...
def generate_kaveri_station_figures(target_stations,
                                     discharge_path='kaveri_data/cauv_discharge.csv',
    ...
    out_dir = f"kaveri_station_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for gap in gap_lengths:
        print(f"\n{'=' * 20} {gap}-day gap: Kaveri distance-only PIMF {'=' * 20}")
        ...
        df_imp_full = run_ordered_missforest_chaining(
            ...
            distance_matrix, zero_connectivity, ordering='most_full_first', random_state=random_state)
    ...
    for station in target_stations:
        ...
        plot_selected_gaps_subplots(df_full_original, data_list, discharge_cols, out_dir,
                                     f"Kaveri_PIMF_{station}", target_station=station)
...
if __name__ == '__main__':
    with warnings.catch_warnings():
        ...
        generate_kaveri_station_figures(['kodum', 'musir', 'urach', 'bilig'])
